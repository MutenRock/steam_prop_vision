...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('Sim Env v4.1 - Presence + Plaque (Config Folder)')
        ...
        # Camera - desactivee par defaut
        self.cam_enabled = tk.BooleanVar(value=False)
        self.cam_source  = tk.StringVar(value='0')  # index entier OU rtsp://
        self._cap        = None
        ...
        self._build_ui()
        self._set_cam_placeholder('Activez PC Cam et cliquez Apply pour demarrer.')
        self._refresh_ui()
    ...
    def _build_ui(self):
        ...
        ttk.Entry(toolbar, textvariable=self.cam_source, width=32).pack(side=tk.LEFT)
        ...
        cam = ttk.LabelFrame(top_right, text='Camera Preview')
        cam.pack(side=tk.LEFT, fill=tk.BOTH, expand=False)
        self.lbl_cam_source = ttk.Label(cam, text='source: -', foreground='gray')
        self.lbl_cam_source.pack(anchor='w', padx=8, pady=(4, 0))
    ...
    def _load_config_folder(self):
        ...
        if not os.path.isdir(plaques_folder):
            messagebox.showwarning('Missing plaques/', 'No plaques folder found at: ' + plaques_folder)
            loaded_ids = []
        else:
        ...
        try:
            ...
            print('[detectors] Loaded: ' + str([type(d).__name__ for d in self._detectors]))
        except Exception as e:
            self._detectors = []
    ...
    def _restart_cam(self):
        ...
        if cv2 is None or ImageTk is None:
            self._set_cam_placeholder('Installez opencv-python + pillow.')
            return
        raw = self.cam_source.get().strip()
        if not raw:
            self._set_cam_placeholder('Source vide. Entrez 0 ou rtsp://...')
            return
        source = int(raw) if raw.isdigit() else raw
        source_label = 'webcam index ' + raw if raw.isdigit() else raw
        cap = cv2.VideoCapture(source)
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
        if not cap.isOpened():
            cap.release()
            self._set_cam_placeholder('Source indisponible : ' + str(source_label))
            return
        ret, frame = cap.read()
        if not ret or frame is None:
            cap.release()
            self._set_cam_placeholder('Flux ouvert mais frame illisible : ' + str(source_label))
            return
        self._cap = cap
        self.lbl_cam_source.configure(text='source: ' + str(source_label))
        print('[cam] Connecte : ' + str(source_label))
    ...
    def _refresh_ui(self):
        ...
        fsm = snap['fsm']
        self.lbl_fsm.configure(text='FSM: ' + str(fsm['state']) + ' locked=' + str(fsm['locked_detection']))
        ld = fsm.get('last_detection')
        if ld:
            self.lbl_last.configure(text='Last injected: ' + str(ld['label']) + ' (' + str(round(ld['conf'], 2)) + ')')
        else:
        ...
        pl = snap['player']
        if pl['playing']:
            self.lbl_player.configure(text='Player: ' + str(pl['name']) + ' remaining ' + str(round(pl['remaining_s'], 1)) + 's')
        else:
